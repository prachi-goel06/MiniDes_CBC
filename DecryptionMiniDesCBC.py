...
# Stream of data to be decrypted
    ...
    init_vector=decryption.dividing_ciphertext_to_block(ciphertext)
    init_vector.insert(0,binary_iv)
    print ("iv value in binary is",binary_iv)
    print ("Message is divided into following blocks: \n", blocks)
    plaintext=""
    for i in range(0, len(blocks)):
        print("Block is ",i+1,": '", blocks[i])

        for j in reversed(range(0,len(key))):
            print('Round ')
            print('Key for round',j+2,'is: ',key[j])
            L = decryption.encryp_step1_L0(blocks[i])
            R = decryption.encryp_step1_R0(blocks[i])
        ...
        Decrypted_Block =(Decrypted_message_Ri + R)
        original_block = decryption.XOR_IV_Block(init_vector[i], Decrypted_Block)
        print("'Decryption of block ",i+1," at the end of 2 round of decryption given by RiLi: '", original_block, '\n\n\n')
        plaintext+=original_block
    print('Plaintext: ', original_block, '\n')
